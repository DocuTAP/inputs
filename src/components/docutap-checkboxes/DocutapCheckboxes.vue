<template>
  <div :class="{'md-input-invalid': errors.has(schema.name)}" class="md-checkboxes-group">
    <div class="md-checkboxes-group-label" v-if="schema.label && (!schema.hideLabel || errors.has(schema.name))">{{schema.label}} <span v-if="errors.has(schema.name)"> {{errors.items[0].rule === 'required' ? 'required' : 'not valid'}}</span></div>
